...
function isSmsConfigured() {
  return !!(process.env.TWILIO_ACCOUNT_SID && process.env.TWILIO_API_KEY_SID && process.env.TWILIO_API_KEY_SECRET && process.env.TWILIO_MESSAGING_SERVICE_SID);
}
      ...
      for (const dogId of dogIds) {
        try {
          ...
          if (autoSelect) {
            ...
            results.push({ dogId, success: true, portraitId: void 0 });
            continue;
          }
          if (!style) {
    ...
    try {
      ...
      for (const dogId of dogIds) {
        try {
          ...
          const portraitImageUrl = latestPortrait ? `${appUrl}/api/portraits/${latestPortrait.id}/image` : void 0;
...
function registerCustomerSessionRoutes(app2) {
  app2.post("/api/customer-session", isAuthenticated, async (req, res) => {
    try {
      ...
      let attempts = 0;
      while (attempts < 10) {
        token = import_crypto.default.randomBytes(4).toString("hex");
...
function verifyMetaSignedRequest(signedRequest) {
  ...
  const expectedSig = import_crypto2.default.createHmac("sha256", IG_APP_SECRET).update(payload).digest("base64url");
  try {
    if (!import_crypto2.default.timingSafeEqual(Buffer.from(sig), Buffer.from(expectedSig))) return null;
  } catch {
    return null;
  }
...
#petportrait #pawtraitpros`;
  ...
  app2.get("/api/instagram-native/connect", isAuthenticated, async (req, res) => {
    ...
    try {
      ...
      const stateHmac = import_crypto2.default.createHmac("sha256", IG_APP_SECRET).update(statePayload).digest("hex");
  ...
  app2.get("/api/instagram-native/callback", async (req, res) => {
    ...
    try {
      ...
      const expectedHmac = import_crypto2.default.createHmac("sha256", IG_APP_SECRET).update(stateOuter.p).digest("hex");
      if (!import_crypto2.default.timingSafeEqual(Buffer.from(stateOuter.s, "hex"), Buffer.from(expectedHmac, "hex"))) {
...
#petportrait #pawtraitpros`;
  ...
  app2.post("/api/instagram-native/data-deletion", async (req, res) => {
    try {
      ...
      const confirmationCode = import_crypto2.default.randomUUID();
      res.json({
